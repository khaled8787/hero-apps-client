...
const AllAppsPage = () => {
  ...
  const limit = 12;
  useEffect(() =>{
   fetch(`http://localhost:5000/apps?skip=${currentPage * limit}&limit=${limit}&sort=${sort}&order=${order}&search=${search}`)
   .then(res => res.json())
   .then(data => {
    setApps(data.apps);
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap justify-around gap-3 mb-3">
        {
